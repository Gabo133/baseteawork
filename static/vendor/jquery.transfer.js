...
var Transfer = (function ($) {
    ...
    function transfer(settings) {
        ...
        $(transferId).find(tabItemName).on("click", function () {
            ...
            if (!$(this).is(".tab-active")) {
                ...
                // 清空右侧数量
                $(transferId).find(selectedTotalNum).text("共0 s");
                // 未选中项
                if ($(transferId).find(tabContentFirst).css("display") != "none") {
                    ...
                    $(transferId).find(groupTotalNum).empty();
                    $(transferId).find(groupTotalNum).append($(transferId).find(transferDoubleGroupListLiUlLi).length + " s");
                } else {
                    // 分组
                    ...
                    $(transferId).find(totalNum).empty();
                    $(transferId).find(totalNum).append($(transferId).find(transferDoubleListLi).length + " s");
                }
                // 数据变化触发回调
        ...
        $(addSelected).on("click", function () {
            ...
            if ($(transferId).find(tabContentFirst).css("display") != "none") {
                ...
                var new_total_num_str = new_group_total_num + " s";
                // 左侧数量
                $(transferId).find(groupTotalNum).append(new_total_num_str);
                // 右侧数量
                $(transferId).find(selectedTotalNum).text(selected_total_num + " s");
                ...
                // 计算右侧总数
                selected_total_num = selectedItemNum;
                var new_total_num_str = new_total_num + " s";
                // 左侧数量
    ...
    function generateTransfer(inputId, currentTimeStr) {
        var htmlStr =
            ...
            + '<div class="tab-item-name tab-item-name-' + currentTimeStr + '">s</div>'
            + '</div>'
